...
# =====================================================
    ...
    if riasec_scores is None or tci_scores is None:
        ...
        # -------------------------------
        # OCR + AI RECOMMENDATION SECTION
        # -------------------------------
        st.subheader("📑 Upload Marksheet Image")
        uploaded_file = st.file_uploader("Upload marksheet image", type=["jpg", "png", "jpeg"], key="marksheet_upload")
        ...
        if uploaded_file:
            ...
            if st.button("🔍 Run OCR Extraction", key="ocr_btn"):
                df = extract_marks_from_marksheet(temp_path, output_csv="temp_uploads/marksheet_marks.csv")
                st.success("✅ OCR Completed")
                st.write("📊 Extracted Marks:")
                st.dataframe(df)
                st.info("CSV saved as temp_uploads/marksheet_marks.csv")

        st.divider()

        # -------- LLM Career Recommendation --------
        st.subheader("🤖 AI Career Recommendation")
        p_csv = st.file_uploader("Upload Personality CSV (from tests)", type=["csv"], key="p_csv_dash")

        if st.button("✨ Recommend Best Career", key="recommend_btn"):
            if p_csv is None:
                st.error("⚠ Please upload Personality CSV.")
            elif uploaded_file is None:
                st.error("⚠ Please upload a marksheet image first.")
            else:
                csv_path = "temp_uploads/uploaded_personality.csv"
                with open(csv_path, "wb") as f:
                    f.write(p_csv.getbuffer())

                best_field, subfields = recommend_field(csv_path, "temp_uploads/marksheet_marks.csv")
                st.success(f"🎯 **Recommended Field:** {best_field}")
                st.write("Top Subfields:")
                for s in subfields:
                    st.write("•", s)

        # ✅ Show top interests & traits
        ...
        st.divider()
        if st.button("✨ Want more personalized results?", key="personalized_btn"):
